...
승인된 목차로 초안을 작성한다. 문체는 `CLAUDE.md §9` 격식체. MD 작성 규칙은 `CLAUDE.md §4-1A`를 따른다. 모든 법령·판결·직접인용은 Phase 3 검증 대상으로 표시한다.
...
- 사용자가 시연·강의·실습 목적이라고 명시하면 `/jurisupport:offline-law-fallback`을 사용해 플러그인 내장 법령 전문 스냅샷에서 조문을 확인할 수 있다.
