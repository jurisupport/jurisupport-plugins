...
프로필 draft를 올리려면 먼저 JuriSupport 웹에서 가입·로그인 후 `/lawyer-search/profile/consent` 동의 페이지를 완료해야 합니다. 시작 URL은 `https://jurisupport.com/signup?redirect=/lawyer-search/profile/consent` 입니다.

이 스킬은 프로필을 자동 공개하거나 의뢰인을 배정하지 않습니다. 변호사를 추천, 알선, 순위화, 광고, 스폰서 노출하지도 않습니다. 원본 사건기록 파일은 자동 업로드하지 않고, 변호사가 확인한 draft JSON만 선택적으로 올립니다. 웹 동의가 없으면 업로드 API가 거절합니다.
...
# korean-law MCP 설치 (법령·판결 1차 검증)
# 법제처 OC 발급: ../../guides/07_law_openapi_key.md
...
0.2.6 - 프로필 draft 업로드 전 웹 동의 절차 반영
